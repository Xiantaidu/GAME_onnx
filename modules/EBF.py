...
from modules.RoPosEmb_s2 import SingleRoPosEmb
...
class AttnWROPEX(nn.Module):
    def __init__(self, dim, num_heads, head_dim, use_rope=True, rope_cache=True, dropout_attn: float = 0.0,
                 out_drop: float = 0.0):
        super().__init__()
    ...
    def forward(self, x):
        ...
        with torch.backends.cuda.sdp_kernel():
            out = F.scaled_dot_product_attention(q, k, v, dropout_p=self.dropout_attn,
                                                 )
...
class PAC(nn.Module):
    def __init__(self, dim, num_heads, head_dim, c_kernel_size=31, m_kernel_size=31, use_rope=True, rope_cache=True,
        ...
        self.c = CgMLP(dim, kernel_size=c_kernel_size,
        ...
        self.merge_dw_conv = nn.Conv1d(dim * 2, dim * 2, kernel_size=m_kernel_size, stride=1,
                                       padding=m_kernel_size // 2,
                                       groups=dim * 2) if m_kernel_size != 0 else None
...
class EBF(nn.Module):
    def __init__(self, dim, num_heads, head_dim, c_kernel_size=31, m_kernel_size=31, use_rope=True, rope_cache=True,
        ...
        super().__init__()
        self.ffn1 = GLUFFN(dim, latent_dim=dim * 4, dropout_latent=ffn_latent_drop,
                           dropout_output=ffn_out_drop) if ffn_type == 'glu' else FFN(dim, latent_dim=dim * 4,
                                                                                      dropout_latent=ffn_latent_drop,
                                                                                      dropout_output=ffn_out_drop)
        self.ffn2 = GLUFFN(dim, latent_dim=dim * 4, dropout_latent=ffn_latent_drop,
                           dropout_output=ffn_out_drop) if ffn_type == 'glu' else FFN(dim, latent_dim=dim * 4,
                                                                                      dropout_latent=ffn_latent_drop,
                                                                                      dropout_output=ffn_out_drop)
        self.attn = PAC(dim, num_heads, head_dim, c_kernel_size, m_kernel_size, use_rope, rope_cache, dropout_attn,
...
class EBFBackbone(nn.Module):
    ...
    输入: x [B, T, indim]
    输出: features [B, T, outdim], boundaries [B, T]
    """
    def __init__(self, indim: int, outdim: int, 
        ...
        self.feature_head = nn.Linear(dim, outdim)  # -> [B, T, outdim]
        self.boundary_head = nn.Linear(dim, 1)      # -> [B, T, 1]
    
    def forward(self, x, mask=None):
        """
        Args:
            x: [B, T, indim] 输入特征 (Mel频谱)
            mask: [B, T] 可选的mask
        Returns:
            features: [B, T, outdim] 用于余弦相似度计算
        ...
        # 特征和边界预测
        features = self.feature_head(x)  # [B, T, outdim]
        boundaries = torch.sigmoid(self.boundary_head(x)).squeeze(-1)  # [B, T]
